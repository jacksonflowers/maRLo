...
class CustomRecordVideo(gym.Wrapper, gym.utils.RecordConstructorArgs):
    ...
    def __init__(
        ...
        self.video_recorder: Optional[custom_video_recorder.CustomVideoRecorder] = None
        ...
        if self.recording:
            ...
            if self.video_length > 0:
                if self.recorded_frames > self.video_length:
                    self.close_video_recorder(info['final_fitness'])
    ...
    def start_video_recorder(self):
        ...
        self.video_recorder = custom_video_recorder.CustomVideoRecorder(
            env=self.env,
            base_path=base_path,
    ...
    def step(self, action):
        ...
        if not (self.terminated or self.truncated):
            ...
            if self.recording:
                ...
                if self.video_length > 0:
                    if self.recorded_frames > self.video_length:
                        self.close_video_recorder(infos['final_fitness'])
                else:
                    if not self.is_vector_env:
                        if terminateds or truncateds:
                            self.close_video_recorder(infos['final_fitness'])
    ...
    def close_video_recorder(self, final_fitness=0):
        """Closes the video recorder if currently recording."""
        if self.recording:
            assert self.video_recorder is not None
            self.video_recorder.close(final_fitness)
        self.recording = False
        self.recorded_frames = 1
